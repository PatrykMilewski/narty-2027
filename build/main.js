...
let mode = 'flight';
const base = { '16–23.01.2027': 1950 * 4.3 / 4, '23–30.01.2027': 2210 * 4.3 / 4, '06–13.02.2027': 2907 * 4.3 / 4, '06–13.03.2027': 2648 * 4.3 / 4 };
const ses = (r) => /Sestriere|Vialattea/.test(r[0]);
const value = (id) => $(id).value;
function fuel() {
    const inputs = ['distance', 'consumption', 'fuelprice', 'people'].map(id => $(id));
    if (inputs.some(input => !input.checkValidity() || !Number.isFinite(input.valueAsNumber)))
        return null;
...
function cost(r) {
    ...
    if (!ses(r))
        return typeof lodging === 'number' ? lodging : null;
    const f = fuel();
