...
const dateLabel = (date) => new Date(date).toLocaleDateString('pl-PL', { day: '2-digit', month: '2-digit', timeZone: 'UTC' });
export function cumulative(monthly, date) {
    let remaining = (date - start) / day;
    return monthly.reduce((sum, snow, i) => {
...
export function renderSnow() {
    ...
    <div class="snow-picker"><div class="snow-picker-heading"><label for="snow-week">Początek tygodnia</label><output id="snow-week-label" for="snow-week"></output></div><input id="snow-week" type="range" min="0" max="${weeks.length - 1}" step="1" value="5"><div class="snow-range-labels" aria-hidden="true"><span>${dateLabel(weeks[0])}</span><span>${dateLabel(weeks[weeks.length - 1])}</span></div></div>
    ...
    const picker = document.getElementById('snow-week');
    const draw = () => {
        const chosen = weeks[Number(picker.value)];
        const weekLabel = `${dateLabel(chosen)}–${dateLabel(chosen + 7 * day)}.2027`;
        document.getElementById('snow-week-label').textContent = weekLabel;
        picker.setAttribute('aria-valuetext', weekLabel);
        const x = (date) => 40 + (date - weeks[0]) / (weeks[12] - weeks[0]) * 270;
        const y = (snow) => 195 - snow / 450 * 170;
